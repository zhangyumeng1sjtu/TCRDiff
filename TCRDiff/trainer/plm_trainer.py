...
from ..model import TCRDPLM, PeptideLM
from ..model.model_utils import EarlyStopping
from .loss import Coord2SeqCrossEntropyLoss, RDMCrossEntropyLoss


class PeptideMLMTrainer(BaseTrainer):
    def __init__(self, config):
        super(PeptideMLMTrainer, self).__init__(config)
        ...
        # configure criterion
        self.loss_fn = Coord2SeqCrossEntropyLoss(ignore_index=0) # ignore padding index
        self.mlm_prob = config.training.mask_prob
    ...
    def step(self, batch):
        ...
        logits = self.model(noised_tokens)
        loss, logging_output = self.loss_fn(logits, tokens, label_mask=noise_mask)
...
class TCRMLMTrainer(BaseTrainer):
    
    def __init__(self, config):
        super(TCRMLMTrainer, self).__init__(config)
        
        self.tokenizer = PairCDR3Tokenizer()
        
        self.model = TCRDPLM(config.model)
        self.model.to(self.device)
    ...
    @torch.no_grad()
    def inject_noise(self, tokens, weights=None):
        
        padding_idx = self.tokenizer.padding_idx
        mask_idx = self.tokenizer.mask_idx

        def _mlm_mask(inputs, weights=None):
            prev_tokens = inputs.clone()
            labels = inputs.clone()
            
            if weights is not None:
                weights = weights.clone()
                probability_matrix = torch.full(labels.shape, self.mlm_prob).to(inputs.device) * weights
            else:
                probability_matrix = torch.full(labels.shape, self.mlm_prob).to(inputs.device)
            special_tokens_mask = (
                prev_tokens.eq(padding_idx)  # & mask
        ...
        prev_tokens, prev_tokens_mask = _mlm_mask(tokens, weights)
    ...
    def step(self, batch):
        ...
        noised_tokens, noise_mask = self.inject_noise(tokens)
        # mlm mask multiply with position weights (0.5-2.0)
        # noised_tokens, noise_mask = self.inject_noise(tokens, weights=batch['entropy_weights'])
        
        # timestep = torch.full((batch['cdr3_token'].size(0),), int(self.mlm_prob * self.model.num_diffusion_steps), device=batch['cdr3_token'].device)
        
        logits = self.model(noised_tokens, batch['chain_token_mask'], batch['cdr12_alpha_feat'], batch['cdr12_beta_feat'])
        loss, logging_output = self.loss_fn(logits, tokens, label_mask=noise_mask)
        # loss, logging_output = self.loss_fn(logits, tokens, label_mask=noise_mask, weights=batch['entropy_weights'])
    ...
    def fit(self, train_loader, val_loader):
        ...
        for epoch in range(self.max_epochs):
            ...
            self.logger.info(f"Epoch: {epoch+1:02} | Epoch Time: {epoch_secs:.2f}s")
            self.logger.info(f"Train Loss: {train_res['loss']:.4f} | NLL Loss: {train_res['nll_loss']:.4f} | Perplexity: {train_res['ppl']:.3f} | ACC: {train_res['acc']*100:.2f}%")
            self.logger.info(f"Valid Loss: {val_res['loss']:.4f} | NLL Loss: {val_res['nll_loss']:.4f} | Perplexity: {val_res['ppl']:.3f} | ACC: {val_res['acc']*100:.2f}%")
    ...
    def forward(self, batch, return_ids=False):
        tokens = batch.pop('cdr3_token')
        noised_tokens, noise_mask = self.inject_noise(tokens)
        batch['cdr3_token'] = noised_tokens
...
class TCRDPLMTrainer(BaseTrainer):
    
    def __init__(self, config):
        super(TCRDPLMTrainer, self).__init__(config)
        
        self.tokenizer = PairCDR3Tokenizer()
        
        self.model = TCRDPLM(config.model, config.training.num_diffusion_steps, config.training.rdm_couple)
        
        if config.training.pretrained_model is not None:
            self.model.load_state_dict(torch.load(config.training.pretrained_model, weights_only=True, map_location='cpu'))
        self.model.to(self.device)
        
        # configure criterion
        self.loss_fn = RDMCrossEntropyLoss(ignore_index=0)
        self.partial_chain_prob = config.training.partial_chain_prob
        self.timestep_weighting = config.training.timestep_weighting
        
        self.max_iter = 10
        self.sampling_strategy = 'argmax' # 'gumbel_argmax'
    ...
    #     masked_samples = torch.bernoulli(torch.full((num_samples,), mask_prob, device=chain_token_mask.device)).bool() # B,
    #     selected_chains = torch.randint(1, 3, size=(num_samples,), device=chain_token_mask.device) # either 1 or 2
        
    #     masked_samples_expanded = masked_samples.unsqueeze(-1).expand(num_samples, seq_len)  # B, L
    #     chosen_chain_mask = (chain_token_mask == selected_chains.unsqueeze(-1))  # [B, L]

    #     masked_chains = torch.where(masked_samples, selected_chains, torch.zeros_like(selected_chains))
    ...
    def step(self, batch):
        # mask partial chain and not drop single chain
        if self.partial_chain_prob > 0:
            partial_masks, _ = self.mask_partial_chain(batch['chain_token_mask'])
        else:
            partial_masks = None
        
        logits, target, loss_mask, weights = self.model.compute_loss(batch, partial_masks=partial_masks, weighting=self.timestep_weighting)
    ...
    def predict_step(self, batch, noise='random_mask', max_iter=100):
        ...
        pred_tokens, pred_scores = self.forward(batch, max_iter, self.sampling_strategy, temperature=self.temperature, use_draft_seq=True, decode_seq=False)
